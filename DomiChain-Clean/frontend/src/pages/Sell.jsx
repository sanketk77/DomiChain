...
export default function Sell() {
  ...
  const filtered = properties.filter((p) => {
    const matchesSearch = `${p.name} ${p.city}`.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-16">
      <section className="bg-charcoal text-gold-light py-16 text-center">
        <h1 className="text-4xl font-extrabold text-gold mb-2">🪙 Sell Property</h1>
        <p className="max-w-2xl mx-auto text-gold-light">List your property and attract buyers from across India securely on-chain.</p>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-10 space-y-6">
        ...
        {loading ? (
          <p className="text-gray-500 text-center mt-10">Loading listings...</p>
        ) : filtered.length > 0 ? (
          <motion.div
          ...
          <p className="text-gray-500 text-center mt-6">No listed properties found.</p>
        )}
      </div>
    </div>
  );
